...
@section('title', 'Edit TUK - Admin LSP')
...
@section('content')
    ...
    <div class="bg-white border-2 border-border-strong rounded-lg shadow-neo p-8 max-w-2xl">
        <h1 class="text-2xl font-black font-display text-slate-900 uppercase mb-6">Edit Tempat Uji</h1>

        <form action="{{ route('admin.tempat-uji.update', $tempatUji) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            <div>
                <label class="block text-sm font-bold text-slate-700 mb-1">Nama Tempat Uji</label>
                <input type="text" name="name" value="{{ old('name', $tempatUji->name) }}"
            ...
            <div>
                ...
                <textarea name="address" rows="3"
                    class="w-full px-3 py-2 border-2 border-border-strong rounded focus:outline-none focus:border-primary"
                    required>{{ old('address', $tempatUji->address) }}</textarea>
            ...
            <div>
                ...
                <input type="number" name="capacity" value="{{ old('capacity', $tempatUji->capacity) }}"
                    class="w-full px-3 py-2 border-2 border-border-strong rounded focus:outline-none focus:border-primary">
            ...
            <div class="flex items-center gap-2">
                ...
                <input type="checkbox" name="is_active" value="1" id="is_active"
                    class="size-5 rounded border-2 border-border-strong text-primary focus:ring-primary" {{ $tempatUji->is_active ? 'checked' : '' }}>
