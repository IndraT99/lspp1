...
@section('title', 'Kelola Skema Sertifikasi')

@section('content')
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-black font-display text-slate-900 uppercase">Skema Sertifikasi</h1>
    ...
    <div class="bg-white border-2 border-border-strong rounded-lg shadow-neo overflow-hidden">
        <table class="w-full text-left border-collapse">
            <thead class="bg-slate-50 border-b-2 border-border-strong">
                <tr>
                    <th class="p-4 text-sm font-bold text-slate-500 uppercase tracking-wide">Kode</th>
                    <th class="p-4 text-sm font-bold text-slate-500 uppercase tracking-wide">Nama Skema</th>
            ...
            <tbody class="divide-y divide-border-strong">
                @forelse($schemes as $scheme)
                    <tr class="hover:bg-blue-50/50 transition-colors">
                        <td class="p-4 font-mono text-sm font-bold text-slate-700">{{ $scheme->code }}</td>
                        ...
                        <td class="p-4 text-right space-x-2">
                            <a href="{{ route('admin.schemes.edit', $scheme) }}"
                            ...
                            <form action="{{ route('admin.schemes.destroy', $scheme) }}" method="POST" class="inline-block"
                ...
                @empty
                    <tr>
                        <td colspan="4" class="p-8 text-center text-slate-500 font-medium">Belum ada data skema.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
