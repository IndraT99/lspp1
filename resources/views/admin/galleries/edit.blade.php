...
@section('content')
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-black font-display text-slate-900 uppercase">Edit Foto Galeri</h1>
        ...
        <form action="{{ route('admin.galleries.update', $gallery) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                    class="w-full px-3 py-2 border-2 border-border-strong rounded focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent font-medium text-slate-900">{{ old('description', $gallery->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs font-bold mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <!-- Image -->
            <div>
                <label for="image" class="block text-sm font-bold text-slate-700 mb-2">Ganti Foto (Opsional)</label>
                <div class="mb-4">
                     <img src="{{ asset('storage/' . $gallery->image_path) }}" alt="Current Image" class="h-32 object-cover rounded border border-border-strong">
                </div>
            ...
            <div>
                ...
                <input type="date" name="event_date" id="event_date" value="{{ old('event_date', $gallery->event_date->format('Y-m-d')) }}" required
                    class="w-full px-3 py-2 border-2 border-border-strong rounded focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent font-medium text-slate-900">
                @error('event_date')
                    <p class="text-red-500 text-xs font-bold mt-1">{{ $message }}</p>
                @enderror
